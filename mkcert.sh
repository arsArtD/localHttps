...
openssl x509 -req -CA $outpath/ca.crt -CAkey $outpath/ca.key -in $outpath/server.req -out $outpath/server.crt -days 3650 -extfile $cnfpath/local.ext -sha256 -set_serial 0x1111
openssl pkcs12 -export -out ca.P12 -inkey $outpath/ca.key -in $outpath/ca.crt
